...
class ElectionTest {
    ...
    fun `With 3 nodes, where 2 will promote to Candidate at the same tick, only one will receive a vote from the remaining Follower (and promote), the other will revert to Follower`() {
        ...
        val willBecomeLeader =
            Follower(
                willBecomeLeaderAddress,
                "NodeA",
                network = network,
                peers = Destination.from(listOf(willLoseElectionAddress, remainsFollowerAddress)),
                config = Config(3),
            )
        val willLoseElection =
            Follower(
                willLoseElectionAddress,
                "NodeB",
                network = network,
                peers = Destination.from(listOf(willBecomeLeaderAddress, remainsFollowerAddress)),
                config = Config(3),
            )
        val remainsFollower =
            Follower(
                ...
                "NodeC",
                network = network,
                peers = Destination.from(listOf(willBecomeLeaderAddress, willLoseElectionAddress)),
                config = Config(10),
            )
